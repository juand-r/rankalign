...
def main() -> None:
    ...
    out = ["# Training-run JSON logs -> setting map (TASK 1c)", "",
           ...
           "> These 85 JSONs are the **gemma-2** runs (2026-05-24+ batch) + the single gemma-4 cu **s13**. "
           "> **qwen3.5-9b has NO structured JSON** — its training log is bundled in each HF model repo as "
           "> `training_log.log.gz` instead. gemma-4 cu **s2** JSON is in "
           "> `../provenance-cu-s2-rankalign/models_g4it/training_run_logs/`; cu **s3/s4/s7** have no "
           "> preserved log (HF adapter-only). See `WANDB_AND_LOGS.md` §2.",
           ""]
