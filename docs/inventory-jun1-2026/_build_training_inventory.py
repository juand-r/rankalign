...
def parse_local(name: str) -> dict | None:
    ...
    return {
        ...
        "epoch": int(d["epoch"]),
        "delta": d["delta"],
        "location": "/datastor2/jdr/rankalign/" + ("models2" if d["ver"] == "v7" else "models"),
        "name": name,
        "private": "",
    }
...
def main() -> None:
    ...
    for fn in ["models2_listing.txt", "models_v6_listing.txt"]:
    ...
    def render(vf: str) -> str:
        out = [f"# Training inventory — {vf}{'*  (v7 + v7b)' if vf=='v7' else ''}",
               "",
               "Auto-generated by `_build_training_inventory.py` from `_raw/` source data "
               "(local dirs on `/datastor2/jdr/rankalign/{models2,models}` + HF repo listings).",
               "Epoch cell shows where each checkpoint lives: `L`=local /datastor2, "
               "`latkes`, `TAUR`=TAUR-dev. Empty = checkpoint not found.",
               "",
               "See `SETTINGS_REFERENCE.md` for what each setting means.",
               "",
               "> **[PENDING datastor1]** Local paths here are the /datastor2 copies. Some "
               "older/larger checkpoints may also exist only under `/datastor1/jdr/gv-gap/"
               "rankalign/` (currently unreachable) — second pass will reconcile.",
               ""]
