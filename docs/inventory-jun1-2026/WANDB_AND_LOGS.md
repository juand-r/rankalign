...
4. **Per-EPOCH training loss** — `Epoch [1/3], Loss: 1.3459` → `[2/3] 0.2869` → `[3/3] 0.0721` (qwen example; healthy decrease). This is the only loss **in the stdout transcript** — coarse, 3 points/run. The **full per-step loss curve is in wandb** (the `train/loss` history) — look up the run in **`WANDB_RUN_MAP.md`** to get its URL, where you'll also see `preference_loss`, `nll_validator_loss`, `nll_generator_loss`, score margins, etc. per step (see §3 for the full key list).
...
**Net — where to get loss, by granularity:**
- **Per-epoch loss (3 pts):** in every stdout transcript (above).
- **Full per-step loss curve:** in **wandb** → find the run in **`WANDB_RUN_MAP.md`** and open its URL.
  Available for runs with a *finished* wandb run (rosch/membership gemma-2 well covered; ifeval
  patchy; persona covered). For **gemma-4 cu** there's no cloud run, but `train_s2.log` embeds the
  wandb **offline summary** (sparkline history + final avg_loss). For most **qwen** cells there's no
  wandb at all, so the 3 per-epoch numbers in the `.log.gz` are all you get.

So "loss is in wandb" is correct for the runs `WANDB_RUN_MAP.md` lists as finished; the stdout logs
are the fallback (coarse per-epoch) and the only loss source where wandb is missing/crashed.
