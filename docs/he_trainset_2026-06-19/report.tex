...
for both settings). (iv) s4 (self-tc) was evaluated in both \emph{self} (\texttt{self/no-base})
and \emph{base}-typicality (\texttt{self/base-typ}) --- i.e.\ self $+$ base --- but not in the
neg-typicality modes; base and s2 were evaluated in all four modes (the neg-mode absence for s4
is by design, not a gap). (v) These are
...
\input{humaneval_TRAIN_table_gen_roc.tex}
\input{humaneval_TRAIN_table_val_roc.tex}
\input{humaneval_TRAIN_table_val_acc.tex}
\input{humaneval_TRAIN_table_pearson.tex}
