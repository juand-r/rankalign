#!/usr/bin/env python3
"""
Focused comparison plot: TC-self (s3) vs no-TC (s4) vs TC-neg (s7).
...
def main():
    ...
    # We want to compare s4 vs s3 (self-TC eval) and s4 vs s7 (neg-TC eval)
    ...
    # Left: self-TC eval comparing s4, s3, s11
    ax = axes[0]
    settings_self = ["s4 (RA full)", "s3 (TC-self)", "s11 (TC-self vlo)", "s5 (TC-self basic)", "s6 (TC-self fsx)"]
    ...
    if not data_self.empty:
        ...
        ax.set_title("Self-TC eval: s4 (no TC) vs TC-self variants\nGemma membership, epoch 2")
        # Add s4 reference line
        s4_val = data_self[data_self["setting"] == "s4 (RA full)"]["tc_gen_roc"].values
        if len(s4_val) > 0:
            ax.axvline(s4_val[0], color='green', linestyle='--', alpha=0.6, label='s4 (no TC)')
        ax.legend()

    # Right: neg-TC eval comparing s4, s7, s12
    ax = axes[1]
    settings_neg = ["s4 (RA full)", "s7 (TC-neg)", "s12 (TC-neg vlo)"]
    ...
    if not data_neg.empty:
        ...
        ax.set_title("Neg-TC eval: s4 (no TC) vs TC-neg variants\nGemma membership, epoch 2")
        s4_val = data_neg[data_neg["setting"] == "s4 (RA full)"]["tc_gen_roc"].values
        if len(s4_val) > 0:
            ax.axvline(s4_val[0], color='green', linestyle='--', alpha=0.6, label='s4 (no TC)')
        ax.legend()
    ...
    # Dynamics: self-TC eval for s3, s4, s11
    ax = axes[0]
    self_dyn = gemma_mem[gemma_mem["tc_eval"] == "self"]
    for setting, color, ls in [("s4 (RA full)", "#2ca02c", "-"),
                                ("s3 (TC-self)", "#d62728", "-"),
    ...
    # Dynamics: neg-TC eval for s4, s7, s12
    ax = axes[1]
    ...
    print("\nSelf-TC eval (s4 vs s3 comparison):")
    for _, row in data_self.sort_values("tc_gen_roc", ascending=False).iterrows():
        delta_str = ""
        if "s4" not in row["setting"]:
            s4_row = data_self[data_self["setting"] == "s4 (RA full)"]
            if not s4_row.empty:
                delta = row["tc_gen_roc"] - s4_row.iloc[0]["tc_gen_roc"]
                delta_str = f" (Δ vs s4: {delta:+.4f})"
        print(f"  {row['setting']:<22}: gen_roc={row['tc_gen_roc']:.4f}{delta_str}")

    print("\nNeg-TC eval (s4 vs s7 comparison):")
    for _, row in data_neg.sort_values("tc_gen_roc", ascending=False).iterrows():
        delta_str = ""
        if "s4" not in row["setting"]:
            s4_row = data_neg[data_neg["setting"] == "s4 (RA full)"]
            if not s4_row.empty:
                delta = row["tc_gen_roc"] - s4_row.iloc[0]["tc_gen_roc"]
                delta_str = f" (Δ vs s4: {delta:+.4f})"
        print(f"  {row['setting']:<22}: gen_roc={row['tc_gen_roc']:.4f}{delta_str}")
