...
# precomputed per-problem long tables (rosch / ifeval-OOD): method name -> setting
METHOD_MAP = {"Base": "base", "SFT labelonly 10%": "s1", "RankAlign": "s2",
...
def build_train_vs_test(df):
    """TRAIN (per-problem) vs held-out TEST (per-problem) gen ROC, PMI-base, mean+-SE.

    Two panels (both gemma-2-9b-it), mirroring the original report's train-vs-test figure:
      - membership TRAIN (68 categories) vs rosch TEST (10 categories)
      - ifeval TRAIN (79 prompts)      vs ifeval-OOD TEST (20 prompts)
    Test columns come from the precomputed per-problem long tables (one ROC per task), which is
    the same source the original report used; train columns are this run's per-problem values.
    PMI-base (= base-typicality / self-TC) scoring throughout, so s7 (neg-TC) is not included.
    """
    def test_from_long(csv_path):
        if not csv_path.exists():
            print(f"  [warn train-vs-test] missing {csv_path}")
            return {}
        r = pd.read_csv(csv_path)
        r = r[r["column"] == "PMI base"].copy()
        r["setting"] = r["method"].map(METHOD_MAP)
        out = {}
        for s, g in r.dropna(subset=["setting"]).groupby("setting"):
            v = g["value"].to_numpy()
            out[s] = (float(np.mean(v)), float(np.std(v, ddof=1) / np.sqrt(len(v))) if len(v) > 1 else 0.0, len(v))
        return out

    def emit(panel_name, train_combo, test_csv, test_label, path, note=""):
        test = test_from_long(test_csv)
        tr = df[(df.combo == train_combo) & (df.tc_eval == "self") &
                ((df.epoch == "ep2") | (df.setting == "base"))]
        lines = [f"% Auto-generated by build_report_fix_metrics.py (train-vs-test {panel_name}, per-problem PMI-base)",
                 "\\begin{tabular}{lccc}", "\\toprule",
                 f"\\textbf{{Setting}} & \\textbf{{Train}} & \\textbf{{Test ({test_label})}} & "
                 "\\textbf{$\\Delta$} \\\\", "\\midrule"]
        any_row = False
        for s in ["base", "s1", "s2", "s3", "s4"]:
            ...
            if s in test:
                ...
                lines.append(f"{s} & ${tm:.3f}\\se{{{tse:.3f}}}$ & ${em:.3f}\\se{{{ese:.3f}}}$ & "
                             f"${em - tm:+.3f}$ \\\\")  # Delta = test - train
            else:
                lines.append(f"{s} & ${tm:.3f}\\se{{{tse:.3f}}}$ & --- & --- \\\\")
        lines += ["\\bottomrule", "\\end{tabular}"]
        if any_row:
            path.write_text("\n".join(lines) + "\n")
            print(f"wrote {path}  ({note or panel_name})")

    M = REPO / "metrics-from-scores"
    Mr = REPO / "metrics-from-scores-rerun-only"
    emit("membership", "gemma_membership", Mr / "rosch_v7_9b-it_gen_roc_table_long.csv",
         "rosch", TABLES / "fix_train_vs_test_membership.tex", "rosch test")
    emit("ifeval", "gemma_ifeval", M / "ifeval_v7_ood_9b-it_gen_roc_table_long.csv",
         "ifeval OOD", TABLES / "fix_train_vs_test_ifeval.tex", "ifeval-OOD test")
