...
\caption{Pairwise score-delta distributions (ep2), the original report's diagnostic: candidates
are pooled per (model, dataset, setting) and $\sim$100k random pairs sampled; each panel overlays
$|\textrm{score}_i-\textrm{score}_j|$ for the generator (blue, typicality-corrected) and validator
(orange, log-odds), with mean $\mu$ in the legend. Tighter = better calibrated; a long generator
tail flags score-spread/instability (e.g.\ RankAlign s2 on IFEval).}
...
\caption{Generator vs validator scores, gemma-2-9b-it / membership, ep2 (member green, non-member
red). \textbf{Rows = generator scoring variant}: \emph{none} (raw $\log P$), \emph{tc self}
(own-model typicality), \emph{tc self base} (base-model typicality); \textbf{columns = settings}.
For these gemma-2-9b-it \emph{train}-set evals only base-typicality was computed (self-typicality
/ own-model was run only for the held-out \emph{test} sets, not the train set this scatter uses),
so the \emph{tc self} row has no train data and is marked ``not evaluated''; only \emph{none} and
\emph{tc self base} are available here. (The HumanEval per-problem train eval did include the
own-model self mode, so its scatter has all three rows.)}
\label{fig:scatter}
\end{figure}
...
\input{tables/fix_train_vs_test_membership.tex}
\end{table}
...
\input{tables/fix_train_vs_test_ifeval.tex}
\end{table}
