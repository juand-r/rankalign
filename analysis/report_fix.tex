...
The original report only ran base-typicality evals, so the \emph{tc self} row has no data and is
marked ``not evaluated'' --- only \emph{none} and \emph{tc self base} are available here (same
procedure as the HumanEval report, which additionally has \emph{tc self}).}
\label{fig:scatter}
\end{figure}
