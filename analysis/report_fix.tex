...
(reuses the file/setting matching from \texttt{analyze\_all\_combos.py}), reading
\texttt{/datastor2/jdr/rankalign/outputs-trainset-dynamics/}.
...
\noindent\emph{Note on the remaining figures in this report.} The per-epoch dynamics,
TC-comparison, score-delta and generator--validator scatter plots below are retained from the
original (pooled) computation; the corrected per-problem numbers are the tables and the bar chart
above. The per-category analysis (Section~\ref{sec:percat}) was already per-category and is
unchanged.
...
\input{tables/fix_concordance.tex}
\end{table}
...
\begin{table}[H]
\centering
\caption{Train vs test gen\_roc: gemma-2-9b-it membership/rosch, PMI-base scoring, \textbf{both
columns per-category mean$\pm$SE}. Train = the 68 membership categories; Test = the 10 held-out
rosch categories. $\Delta=$ test$-$train. (s5/s11 are omitted: not present in the per-category
rosch test artifact; s1/s7 use a different scoring direction and are excluded from this PMI-base
comparison.)}
\label{tab:train_test}
\input{tables/fix_train_vs_test.tex}
\end{table}

\textbf{Key finding} (unchanged under per-category recomputation): s3 (full method, no TC)
outperforms s2 on training categories ($0.947$ vs $0.945$) but \emph{underperforms} on the
held-out rosch categories ($0.886$ vs $0.895$). Meanwhile s4 (TC-self) is highest on both splits
($0.974$ train, $0.926$ test). This supports \textbf{TC promotes generalization} --- the non-TC
full method overfits slightly to training-specific patterns while the TC model maintains its
advantage out of distribution.
