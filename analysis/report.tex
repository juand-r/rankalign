...
\caption{Score spread evolution during training. The generator panels include only settings with $\text{NLL}_G > 0$ (s3, s4, s7) since s1/s2 do not log generator scores; the validator panels include s2, s3, s4. Note that s2 on Gemma IFEval shows highly volatile validator scores (top-right), and the loss-breakdown table indicates a very large average $\text{NLL}_G$ for s2 driven by extreme generator log-prob values, even though the generator score is not plotted here.}
