...
\caption{Generator vs validator scores for Base, s4, and s3 (self-TC scoring, gemma membership). Training improves the separation between positive (green) and negative (red) items in both generator and validator space.}
\label{fig:scatter}
\end{figure}
...
\caption{Left: distribution of per-category improvements (Base $\to$ s3). Right: improvements from TC-self specifically (s4 $\to$ s3). Both are consistently positive.}
...
s3 (TC-self full) & \textbf{0.976} & 0.920 & $-$0.056 \\
s5 (TC-self basic) & 0.958 & 0.913 & $-$0.045 \\
s2 (RA basic) & 0.941 & 0.891 & $-$0.050 \\
s4 (RA full) & 0.948 & 0.885 & $-$0.063 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key finding}: s4 (full method, no TC) outperforms s2 on training categories but \emph{underperforms} on test categories (different Rosch categories). Meanwhile, all TC-trained models (s3, s11, s5) maintain their advantage on both splits. This suggests \textbf{TC promotes generalization} --- it prevents overfitting to training-specific patterns.
...
\item \textbf{TC promotes generalization}: TC-trained models maintain their advantage on held-out test categories, while non-TC models (s4) can overfit to training patterns.
