...
# ============================================================================
# Typicality-correction lambda(y) — controls *which* examples get TC applied
# ============================================================================
# Original behavior: gen_score - typicality   (TC on every example)
# Generalized:       gen_score - lambda(y) * typicality
#
# Each lambda function returns a per-example coefficient in [0, 1].
# Drop in a new function and register it in TC_LAMBDA_FUNCTIONS to add a strategy.
#
# Conventions: all functions accept **kwargs and pull out what they need so that
# new strategies can demand new context (e.g. validator score) without changing
# the call site.

def tc_lambda_all(indicator=None, **_):
    """λ(y) = 1 for every example. Recovers original TC-on-everything behavior."""
    if indicator is None:
        raise ValueError(
            "tc_lambda_all expected an `indicator=...` kwarg (per-example "
            "0/1 tensor used to size the output). "
            "Check the call site in compute_tc_lambda(...) for typos."
        )
    return torch.ones_like(indicator)


def tc_lambda_oracle(indicator=None, **_):
    """λ(y) = indicator. Apply TC only to *correct* (positive) examples.

    Oracle proof-of-concept: requires ground-truth labels. Hypothesis:
    boosting incorrect examples via TC washes out their suppression signal,
    so confining TC to positives should improve gen ROC.

    Note: assumes hard binary correctness (1 = positive, 0 = negative). If
    the underlying data has gradations of correctness, this 0/1 assignment
    is lossy — consider a soft proxy (val_threshold/val_sigmoid) instead.
    """
    if indicator is None:
        raise ValueError(
            "tc_lambda_oracle expected an `indicator=...` kwarg "
            "(per-example 0/1 ground-truth label tensor). "
            "Check the call site in compute_tc_lambda(...) for typos."
        )
    return indicator.float()


# NOTE: tc_lambda_val_threshold and tc_lambda_val_sigmoid are not yet reachable
# from the CLI — see TC_LAMBDA_CLI_CHOICES below. They are kept here so that
# wiring `val_score` through the training loop is a one-line change once we
# decide where to compute it.
def tc_lambda_val_threshold(val_score=None, threshold=0.0, **_):
    """λ(y) = 1 if val_score > threshold else 0. Hard semi-supervised proxy.

    `val_score` is detached so the gen-loss does not backprop into the
    validator path through λ.
    """
    if val_score is None:
        raise ValueError(
            "tc_lambda_val_threshold needs `val_score=...` in context. "
            "This strategy is not yet wired into the training loop — "
            "compute val_score (e.g. log-odds at the validator position) "
            "before the typicality-correction block and pass it via "
            "compute_tc_lambda(..., val_score=val_score_i, ...)."
        )
    return (val_score.detach() > threshold).float()


def tc_lambda_val_sigmoid(val_score=None, threshold=0.0, **_):
    """λ(y) = sigmoid(val_score - threshold). Smooth semi-supervised proxy.

    `val_score` is detached so the gen-loss does not backprop into the
    validator path through λ.

    With `--validator-log-odds` and `threshold=0`, λ literally equals the
    validator's P(yes|x) (i.e. its confidence the example is correct). With
    log-prob (non-log-odds) val_score, the threshold becomes scale-dependent
    — set it to log(0.5) ≈ -0.693 for symmetric behavior in that case.
    """
    if val_score is None:
        raise ValueError(
            "tc_lambda_val_sigmoid needs `val_score=...` in context. "
            "This strategy is not yet wired into the training loop — see "
            "tc_lambda_val_threshold for the wiring instructions."
        )
    return torch.sigmoid(val_score.detach() - threshold)


TC_LAMBDA_FUNCTIONS = {
    "all": tc_lambda_all,
    "oracle": tc_lambda_oracle,
    "val_threshold": tc_lambda_val_threshold,
    "val_sigmoid": tc_lambda_val_sigmoid,
}

# Strategies currently exposed via --tc-lambda-strategy. val_*  variants are
# implemented but not yet wired (val_score isn't passed through the training
# loop), so we omit them from the CLI to fail fast on misuse. To enable: wire
# val_score in the typicality-correction block, then add the strategy here.
TC_LAMBDA_CLI_CHOICES = ["all", "oracle"]


def compute_tc_lambda(strategy, **context):
    """Dispatch to the registered λ function for `strategy`.

    Pass any per-example tensors needed by the chosen strategy as keyword args
    in `context` (e.g. indicator=indicator_i, val_score=val_score_i, threshold=...).
    """
    if strategy not in TC_LAMBDA_FUNCTIONS:
        raise ValueError(
            f"Unknown tc_lambda_strategy='{strategy}'. "
            f"Choices: {sorted(TC_LAMBDA_FUNCTIONS)}"
        )
    return TC_LAMBDA_FUNCTIONS[strategy](**context)
...
def get_tracking_base_filename(model_name, task, delta, train_g_or_d, use_all, split_type, alpha,
    ...
    if neg_typicality:
        ...
        typcorr_str = ""
    tc_lambda_str = f"-tc{tc_lambda_strategy}" if (typicality_correction and tc_lambda_strategy != "all") else ""
    ...
    base_name = (f"v5-{model_name.replace('/', '--')}-delta{delta}--{task}{all_str}"
                 f"--{direction_str}--{split_type}{alpha_str}{typcorr_str}{tc_lambda_str}{lenorm_str}"
...
def main(args):
    ...
    if track_scores:
        tracking_base_name = get_tracking_base_filename(
            ...
            tc_lambda_strategy=args.tc_lambda_strategy,
        )
    ...
    if use_wandb:
        ...
        if run_name is None:
            ...
            pref_str = f"-pref{preference_loss_weight}" if preference_loss_weight != 1.0 else ""
            tc_lambda_str = (
                f"-tc{args.tc_lambda_strategy}"
                if (args.typicality_correction and args.tc_lambda_strategy != "all")
                else ""
            )
            if args.semi_supervised is not None:
                ...
                semi_str = ""
            run_name = f"{task}-{train_g_or_d}-delta{delta}-nllv{nll_validator_weight}-nllg{nll_generator_weight}{pref_str}{tc_lambda_str}{semi_str}-lr{lr}"
        ...
        wandb.init(
            ...
            config={
                ...
                "tc_lambda_strategy": args.tc_lambda_strategy,
                "tc_lambda_threshold": args.tc_lambda_threshold,
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(train_loader):
            ...
            if train_g_or_d == 'both':
                ...
                # λ(y) controls *which* examples get TC — see compute_tc_lambda.
                if args.typicality_correction:
                    typicality_i = batch["typicality_i"].to(device)
                    typicality_j = batch["typicality_j"].to(device)
                    lambda_i = compute_tc_lambda(
                        args.tc_lambda_strategy,
                        indicator=label_i,
                        threshold=args.tc_lambda_threshold,
                    )
                    lambda_j = compute_tc_lambda(
                        args.tc_lambda_strategy,
                        indicator=label_j,
                        threshold=args.tc_lambda_threshold,
                    )
                    score_i_gen = score_i_gen - lambda_i * typicality_i
                    score_j_gen = score_j_gen - lambda_j * typicality_j
                ...
                if use_wandb:
                    ...
                    if args.typicality_correction:
                        log_dict["train/tc_lambda_i"] = lambda_i.float().mean().item()
                        log_dict["train/tc_lambda_j"] = lambda_j.float().mean().item()
                    wandb.log(log_dict)
                ...
                if args.typicality_correction and train_g_or_d == 'g':
                    ...
                    score_i = score_i - lambda_i * typicality_i
                    score_j = score_j - lambda_j * typicality_j
        ...
        if epoch%save_steps == 0 or epoch == num_epochs - 1:
            ...
            save_directory = args.models_dir + "/v6-" + model_name.replace('/','--')  + "-delta"+str(delta)+"-epoch"+str(epoch) + "--" + task + with_ref_str + all_str + direction_str + split_type_str + alpha_str + typcorr_str + tc_lambda_str + lenorm_str + single_token_str + full_completion_str + eos_str + pref_str + nll_v_str + nll_g_str + force_same_x_str + valboost_str + vallogodds_str + semi_str
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="all",
        choices=TC_LAMBDA_CLI_CHOICES,
        help=(
            "Which examples get typicality correction applied. "
            "Loss becomes: gen_score - lambda(y) * typicality. "
            "'all' (default): lambda=1 for every example (original behavior). "
            "'oracle': lambda=indicator(label=positive) — TC only on correct examples. "
            "(val_threshold / val_sigmoid strategies are implemented but not yet "
            "wired through the training loop — see TC_LAMBDA_CLI_CHOICES.)"
        ),
    )
    parser.add_argument("--tc-lambda-threshold", type=float, default=0.0, help="Threshold for val_threshold/val_sigmoid lambda strategies (currently unused)")
