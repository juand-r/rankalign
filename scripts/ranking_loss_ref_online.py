...
def main(args):
    ...
    if use_wandb:
        ...
        wandb.init(
            ...
            config={
                ...
                "online_typicality": args.online_typicality,
                "online_pair_selection": args.online_pair_selection,
    ...
    # ---------- helpers used in both pair-payload construction and typicality
    # prompt precomputation. Defined early so both call-sites have access. -----
    ...
    # ---------- ONLINE-TYPICALITY: per-item typicality prompt strings -------
    # If --online-typicality is on, build the per-item typicality-prompt
    # strings (model-independent; the model is queried at loss-step time
    # against these). For self-TC the prompt is "" (empty); for neg-TC the
    # prompt is the negated generator prompt.
    # If --online-typicality is OFF, fill with empty strings — they're never
    # touched. We always include the slot in Z so the structure is uniform.
    if args.online_typicality:
        if task_config is None:
            raise ValueError(
                f"--online-typicality currently only supports tasks in the "
                f"modern task registry. Got task='{task}' which has no registry "
                f"entry."
            )
        if args.neg_typicality:
            from eval_by_claude import make_negated_gen_prompt
            _online_typ_make_prompt_fn = task_config['make_prompt']
            typicality_prompts_for_z = []
            for _item in L_train_all:
                _neg_prompt, _ = make_negated_gen_prompt(_item, task, _online_typ_make_prompt_fn)
                if with_chat:
                    _neg_prompt = format_with_inst(_neg_prompt)
                typicality_prompts_for_z.append(_neg_prompt)
            print(f"[online-typicality] Precomputed {len(typicality_prompts_for_z)} neg-prompts")
        else:
            _empty_typ_prompt = format_with_inst("") if with_chat else ""
            typicality_prompts_for_z = [_empty_typ_prompt] * len(L_train_all)
            print(f"[online-typicality] Using self-TC null-context prompts ("
                  f"{'chat-formatted empty' if with_chat else 'empty string'}); "
                  f"{len(typicality_prompts_for_z)} items")
    else:
        typicality_prompts_for_z = [""] * len(L_train_all)
    ...
    if args.semi_supervised is not None or args.labeled_only is not None:
        ...
        if args.labeled_only is not None:
            ...
            typicality_prompts_for_z = [typicality_prompts_for_z[i] for i in keep]
            is_labeled_flags = [True] * len(L_train_all)
            print(f"Filtered to {len(L_train_all)} labeled items")
        print(f"{'='*60}\n")

    if train_g_or_d == 'both':
        # Create tuples of (discriminator_prompt, generator_prompt, logprobs, typicality, is_labeled, typicality_prompt)
        # Note: logprobs_last_layer contains tuples of (log_prob_d, log_prob_g)
        Z = list(zip(p_train_tune, p_train_gold, logprobs_last_layer, typ_scores_for_z, is_labeled_flags, typicality_prompts_for_z))
        ...
        # Also include typicality scores (index 3) and typicality prompts (index 5)
        # (p_train_tune, logprobs, L_train_all, typicality, is_labeled, typicality_prompt)
        Z = list(zip(p_train_tune, logprobs_last_layer, L_train_all, typ_scores_for_z, is_labeled_flags, typicality_prompts_for_z))
    ...
    if train_g_or_d=='d':
        ...
        if with_chat:
             pairs = [
                 (
                     ...
                     (pair[0][5], pair[1][5]),  # typicality prompts (online TC)
                 )
        ...
        if with_chat:
            ...
            # Z structure: (p_train_tune, p_train_gold, logprobs, typicality, is_labeled, typicality_prompt)
            pairs = [
                (
    ...
    class PairwiseDataset(Dataset):
        ...
        def __getitem__(self, idx):
            if train_g_or_d == 'both':
                # 6-element structure: (disc_pair, gen_pair, labels, typicality, is_labeled, typicality_prompt)
                ((prompt_i_disc, prompt_j_disc), (completion_i_disc, completion_j_disc)), ((prompt_i_gen, prompt_j_gen), (completion_i_gen, completion_j_gen)), (label_i, label_j), (typicality_i, typicality_j), (is_labeled_i, is_labeled_j), (typicality_prompt_i, typicality_prompt_j) = self.pairs[idx]
                ...
                # 8-element pair structure: (prompts, ranking_completions, validator_correct, gen_completions, indicators, typicality, is_labeled, typicality_prompt)
                (prompt_i, prompt_j), (completion_i, completion_j), (correct_i, correct_j), (gen_completion_i, gen_completion_j), (indicator_i, indicator_j), (typicality_i, typicality_j), (is_labeled_i, is_labeled_j), (typicality_prompt_i, typicality_prompt_j) = self.pairs[idx]
            ...
            #    label_j = "yes" if "yes" in prompt_j.lower() else "no"
                ...
                # ----- ONLINE-TYPICALITY: tokenize (typicality_prompt + completion) -----
                # Used in the loss step when --online-typicality is on. The completion
                # used here MUST be `completion_i` (the same one fed into the regular
                # gen-score path — it corresponds to token_id_i), so the same
                # `token_id_i` indices can be used to gather the typicality logprobs.
                # Only the conditioning context differs: "" (or chat-empty) for
                # self-TC, neg(x) for neg-TC.
                if args.online_typicality:
                    typ_input_i = typicality_prompt_i + completion_i
                    typ_input_j = typicality_prompt_j + completion_j
                    enc_i_typ = self.tokenizer(
                        typ_input_i,
                        padding='max_length',
                        truncation=True,
                        max_length=self.max_length,
            ...
            if train_g_or_d != 'both':
                ...
                item = {
                    ...
                    'typicality_i': torch.tensor(typicality_i, dtype=torch.float),  # cached log P_base(y_i) (offline TC)
                    'typicality_j': torch.tensor(typicality_j, dtype=torch.float),  # cached log P_base(y_j) (offline TC)
                    'is_labeled': torch.tensor(pair_is_labeled, dtype=torch.float),
                }
                if args.online_typicality:
                    # Tokens needed for live typicality forward pass (same completion
                    # tokens as token_gen_i/j; only the conditioning context differs).
                    item['typ_input_ids_i'] = enc_i_typ['input_ids'].squeeze(0)
                    item['typ_attention_mask_i'] = enc_i_typ['attention_mask'].squeeze(0)
                    item['typ_input_ids_j'] = enc_j_typ['input_ids'].squeeze(0)
                    item['typ_attention_mask_j'] = enc_j_typ['attention_mask'].squeeze(0)
            else:
                pair_is_labeled = 1.0 if (is_labeled_i and is_labeled_j) else 0.0
                item = {
    ...
    # =========================================================================
    # ONLINE PAIR SELECTION HELPER
    # =========================================================================
    # Restrictions: modern task registry only; g-mode or d-mode (not 'both');
    # batch_size > 1 + semi/labelonly known-buggy combo is still rejected by
    # the original check above (we keep the same batch_size).
    def _rebuild_train_loader_online(current_model):
        """Recompute validator/gold scores against current_model and rebuild
        the DataLoader from scratch (Z, pair_inds, pairs payload, dataset).
        Returns the new train_loader. See docs/IMPORTANT-RESEARCH-PLAN.md §3a.
        """
        if task_config is None:
            raise ValueError(
                "--online-pair-selection requires the modern task registry"
            )
        if train_g_or_d not in ('g', 'd'):
            raise ValueError(
                f"--online-pair-selection only supports g/d mode (got {train_g_or_d})"
            )

        # 1) Recompute logprobs_last_layer (gold scores) under current model.
        was_training = current_model.training
        current_model.eval()
        new_logprobs = []
        with torch.no_grad():
            for idx, prompt in enumerate(tqdm(prompts_gold, desc="[online-pairs] recompute gold")):
                if train_g_or_d == 'd':
                    target_text = space_prefix + task_config['get_completion'](L_train_all[idx]).strip()
                else:  # 'g'
                    target_text = space_prefix + "Yes"
        ...
        # 2) Rebuild Z and sort.
        Z_new = list(zip(p_train_tune, new_logprobs, L_train_all, typ_scores_for_z,
                         is_labeled_flags, typicality_prompts_for_z))
        Z_new = sorted(Z_new, key=lambda i: i[1])

        # 3) Rebuild pair_inds.
        if args.force_same_x:
            prompt_to_indices = defaultdict(list)
            for idx_, z in enumerate(Z_new):
                prompt_to_indices[z[0].prompt].append(idx_)
            prompt_to_valid_pairs = {}
            total_valid_pairs = 0
            for prm, indices in prompt_to_indices.items():
                group_pairs = []
                for ii, jj in itertools.combinations(indices, 2):
                    li, lj = Z_new[ii][1], Z_new[jj][1]
                    if abs(li - lj) > delta:
                        group_pairs.append((ii, jj) if li < lj else (jj, ii))
                prompt_to_valid_pairs[prm] = group_pairs
                total_valid_pairs += len(group_pairs)
            if total_valid_pairs == 0:
                raise ValueError(
                    f"[online-pairs] No valid pairs after delta filter (delta={delta}). "
                    f"Try reducing --delta."
                )
            this_round_total = min(total_samples, total_valid_pairs)
            pair_inds_new = []
            remaining_budget = this_round_total
            sorted_groups = sorted(prompt_to_valid_pairs.items(), key=lambda x: len(x[1]))
            remaining_groups = len(sorted_groups)
            for prm, pp in sorted_groups:
                fair_share = remaining_budget // max(remaining_groups, 1)
                n_samples = min(len(pp), fair_share)
                pair_inds_new.extend(random.sample(pp, n_samples))
                remaining_budget -= n_samples
                remaining_groups -= 1
            random.shuffle(pair_inds_new)
        else:
            indices = range(len(Z_new))
            all_inds = [(i, j) for i, j in itertools.product(indices, repeat=2) if i < j]
            pair_inds_new = random.sample(all_inds, min(total_samples, len(all_inds)))
            # Ensure i is the lower-logprob (loser) and j is higher (winner)
            pair_inds_new = [(i, j) if Z_new[i][1] < Z_new[j][1] else (j, i)
                             for (i, j) in pair_inds_new]

        pairs_local = [(Z_new[i[0]], Z_new[i[1]]) for i in pair_inds_new]

        # 4) Rebuild pairs payload (g-mode and d-mode only).
        completion_text_local = space_prefix + "Yes"
        if train_g_or_d == 'd':
            if with_chat:
                pairs_new = [
                    (
                        (format_with_inst(pp[0][0].prompt), format_with_inst(pp[1][0].prompt)),
                        (completion_text_local, completion_text_local),
                        (get_correct_answer(pp[0][2], task), get_correct_answer(pp[1][2], task)),
                        (get_generator_completion(pp[0][2], task), get_generator_completion(pp[1][2], task)),
                        (get_indicator(pp[0][2], task), get_indicator(pp[1][2], task)),
                        (pp[0][3], pp[1][3]),
                        (pp[0][4], pp[1][4]),
                        (pp[0][5], pp[1][5]),
                    )
                    for pp in pairs_local if pp[1][1] - pp[0][1] > delta
                ]
            else:
                pairs_new = [
                    (
                        (pp[0][0].prompt, pp[1][0].prompt),
                        (completion_text_local, completion_text_local),
                        (get_correct_answer(pp[0][2], task), get_correct_answer(pp[1][2], task)),
                        (get_generator_completion(pp[0][2], task), get_generator_completion(pp[1][2], task)),
                        (get_indicator(pp[0][2], task), get_indicator(pp[1][2], task)),
            ...
            if with_chat:
                pairs_new = [
                    (
                        (format_with_inst(pp[0][0].prompt), format_with_inst(pp[1][0].prompt)),
                        (pp[0][0].completion, pp[1][0].completion),
                        (get_correct_answer(pp[0][2], task), get_correct_answer(pp[1][2], task)),
                        (get_generator_completion(pp[0][2], task), get_generator_completion(pp[1][2], task)),
                        (get_indicator(pp[0][2], task), get_indicator(pp[1][2], task)),
                        (pp[0][3], pp[1][3]),
        ...
        # 5) Rebuild dataset and DataLoader.
        new_dataset = PairwiseDataset(
            pairs_new, tokenizer,
            max_length=max_context_length, device=device,
            use_full_completion=use_full_completion,
        )
        new_loader = DataLoader(new_dataset, batch_size=batch_size, shuffle=True)
        print(f"[online-pairs] Rebuilt loader: {len(pairs_new)} pairs "
              f"({len(pair_inds_new)} candidates, delta={delta})")
        return new_loader
    # =========================================================================

    # Sanity check at startup so we fail fast.
    if args.online_pair_selection and (task_config is None or train_g_or_d not in ('g', 'd')):
        raise ValueError(
            f"--online-pair-selection requires task_config (modern registry) "
            f"and train_g_or_d in (g, d). Got task_config={task_config}, "
            f"train_g_or_d={train_g_or_d}."
        )

    for epoch in range(num_epochs):
        # Online pair selection: at the start of each epoch (after epoch 0),
        # recompute validator scores under the CURRENT model and rebuild the
        # DataLoader. Skips epoch 0 because the initial loader is already
        # built from the initial-model scores.
        if args.online_pair_selection and epoch > 0:
            print(f"\n[online-pairs] Epoch {epoch}: rebuilding loader from current model...")
            train_loader = _rebuild_train_loader_online(model)
        model.train()
        total_loss = 0.0
        ...
        for batch in tqdm(train_loader):
            ...
            if train_g_or_d == 'both':
                ...
                # Apply typicality correction to generator scores (only g-mode).
                # Two paths:
                #   - args.online_typicality: forward pass through current model
                #     with grads enabled; t_i depends on θ → injects gradient
                #     direction signal.
                #   - else (default): use the precomputed cached scalar (offline TC),
                #     identical to ranking_loss_ref.py behavior.
                if args.typicality_correction and train_g_or_d == 'g':
                    if args.online_typicality:
                        typ_input_ids_i = batch["typ_input_ids_i"].to(device)
                        typ_attention_mask_i = batch["typ_attention_mask_i"].to(device)
                        typ_input_ids_j = batch["typ_input_ids_j"].to(device)
                        typ_attention_mask_j = batch["typ_attention_mask_j"].to(device)

                        outputs_i_typ = model(input_ids=typ_input_ids_i, attention_mask=typ_attention_mask_i, use_cache=False)
                        outputs_j_typ = model(input_ids=typ_input_ids_j, attention_mask=typ_attention_mask_j, use_cache=False)
                        log_probs_i_typ = F.log_softmax(outputs_i_typ.logits, dim=-1)
                        log_probs_j_typ = F.log_softmax(outputs_j_typ.logits, dim=-1)

                        # Use the same length normalization as the gen score so the
                        # subtraction is between commensurable quantities.
                        use_lenorm_typ = args.length_normalize and train_g_or_d == 'g'
                        # Use token_id_i/j (same completion tokens as the gen-score
                        # path) to gather the typicality logprobs.
                        typicality_i_live = sum_completion_logprobs(
                            log_probs_i_typ, token_id_i, length_normalize=use_lenorm_typ
                        )
                        typicality_j_live = sum_completion_logprobs(
                            log_probs_j_typ, token_id_j, length_normalize=use_lenorm_typ
                        )
                        score_i = score_i - typicality_i_live
                        score_j = score_j - typicality_j_live
                    else:
                        typicality_i = batch["typicality_i"].to(device)
                        typicality_j = batch["typicality_j"].to(device)
        ...
        if epoch%save_steps == 0 or epoch == num_epochs - 1:
            ...
            online_pairs_str = "--online-pairs" if args.online_pair_selection else ""
            online_tc_str = "--online-tc" if args.online_typicality else ""
            if args.semi_supervised is not None:
                semi_str = f"--semi{args.semi_supervised}"
            elif args.labeled_only is not None:
                semi_str = f"--labelonly{args.labeled_only}"
            ...
            save_directory = args.models_dir + "/v6-" + model_name.replace('/','--')  + "-delta"+str(delta)+"-epoch"+str(epoch) + "--" + task + with_ref_str + all_str + direction_str + split_type_str + alpha_str + typcorr_str + lenorm_str + single_token_str + full_completion_str + eos_str + pref_str + nll_v_str + nll_g_str + force_same_x_str + valboost_str + vallogodds_str + online_pairs_str + online_tc_str + semi_str
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Recompute validator (gold) scores against the CURRENT model at the start "
             "of each epoch (after epoch 0) and rebuild the pair dataset/dataloader. "
             "Default: off (current behavior, scores frozen at initial model). "
             "Requires the task to be in the modern task registry.",
    )
    parser.add_argument(
        "--online-typicality",
        action="store_true",
        default=False,
        help="Compute typicality scores LIVE during the loss step against the current "
             "trainable model with grads enabled, instead of the precomputed cached "
             "scalars. Requires --self-typicality or --neg-typicality. Makes the "
             "typicality term θ-dependent so it actually injects a gradient-direction "
             "signal (see docs/IMPORTANT-RESEARCH-PLAN.md §3a, May 10 finding).",
    )
    ...
    if args.neg_typicality:
        args.typicality_correction = True
    if args.online_typicality and not (args.self_typicality or args.neg_typicality):
        parser.error("--online-typicality requires --self-typicality or --neg-typicality")
    if args.online_typicality and args.train_g_or_d != 'g':
        parser.error("--online-typicality currently only supports --train_g_or_d g")
