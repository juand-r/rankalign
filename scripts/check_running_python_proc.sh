#!/bin/bash
# Check for python processes run by jdr on mll-a40-1 through mll-a40-8
#
# RECOMMENDED: First run ssh-add to cache your passphrase:
#   ssh-add ~/.ssh/id_rsa
#   (enter passphrase once)
#   ./check_running_python_proc.sh
#
# OR with passphrase inline: SSH_PASS="yourpassphrase" ./check_running_python_proc.sh

OUTPUT_FILE="python_proc_status.txt"
...
if ssh-add -l &>/dev/null; then
    ...
    if [ -z "$SSH_PASS" ]; then
        ...
        echo "  2. Run with: SSH_PASS=\"yourpassphrase\" ./check_running_python_proc.sh"
        echo ""
...
echo "Checking python processes for jdr on mll-a40 servers..."
...
for N in 1 2 3 4 5 6 7 8; do
    ...
    if [ "$USE_EXPECT" = true ]; then
        ...
        expect -c "
            log_user 0
            set timeout 15
            spawn ssh -o StrictHostKeyChecking=no $SERVER {ps aux | grep '^jdr' | grep python | grep -v grep}
            expect {
        ...
        ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no "$SERVER" "ps aux | grep '^jdr' | grep python | grep -v grep" >> "$OUTPUT_FILE" 2>&1
    fi
