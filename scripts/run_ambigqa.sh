...
# Train and evaluate models on the ambigqa task (same layout as run_ifeval_concat.sh).
#
...
#
# Evaluates each variant on the explicit ambigqa test task list from
# scripts/semi_supervised_eval_runs_ambigqa.sh (avoids picking up other ambigqa-* tasks).
#
# Usage:
#   ./run_ambigqa.sh <NUM_GPUS> [--model MODEL] [--task TASK] [--tc]
...
if [ -z "$NUM_GPUS_INPUT" ]; then
    ...
    echo "  --task: training task name (default: ambigqa)"
...
# scripts/semi_supervised_eval_runs_ambigqa.sh TASKS= (explicit test split)
EVAL_TASKS_STR="ambigqa-american ambigqa-danube ambigqa-executed ambigqa-gives ambigqa-harry ambigqa-involved ambigqa-jack ambigqa-plays ambigqa-received ambigqa-sang ambigqa-soccer ambigqa-used ambigqa-voice ambigqa-winter ambigqa-won ambigqa-world ambigqa-year"
read -ra EVAL_TASKS <<< "$EVAL_TASKS_STR"

if [ ${#EVAL_TASKS[@]} -eq 0 ]; then
    echo "No ambigqa eval tasks configured"
    exit 1
fi
