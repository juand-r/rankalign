#!/bin/bash
# run_rerun_9bit_membership_wandb.sh
#
# Re-train google/gemma-2-9b-it x membership (delta-bins) for settings
# s1 s2 s3 s4 s7 WITH wandb logging enabled, into a SEPARATE checkpoint dir
# and with TAGGED wandb run names — so the new runs are trivially distinguishable
# from the paper artifacts.
#
# Why this exists: the paper's gemma-2-9b-it x membership delta-bins checkpoints
# were trained with an older code version (pre-v7, no fix1/fsx/ppd/vallogodds).
# These reruns reproduce the v7 configs through the mll launcher
...
#   3. WandB runs   -> name "rerun-wandb-gemma-2-9b-it-membership-<setting>"
#                      (project "rankalign"; filter by the "rerun-wandb-" prefix)
#
# Usage:
#   bash scripts/run_rerun_9bit_membership_wandb.sh                 # submit all 5
#   DRYRUN=1 bash scripts/run_rerun_9bit_membership_wandb.sh        # print only, no submit
#   SETTINGS="s1 s2" bash scripts/run_rerun_9bit_membership_wandb.sh  # subset
#
# Env overrides:
...
#   SETTINGS      space-separated  (default "s1 s2 s3 s4 s7")
...
# Old membership runs finished in 3-6h; v7 with fix1/fsx/ppd may be slightly
# longer but 9h should be plenty.
export WALLTIME="${WALLTIME:-9}"

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"

echo "============================================================"
echo "RERUN: $MODEL x $DATASET (membership-sans-rosch-v0, eval on rosch, wandb ON)"
...
echo
echo "=== all settings dispatched ($DATASET): $SETTINGS ==="
