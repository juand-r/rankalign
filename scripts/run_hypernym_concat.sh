...
# Train and evaluate models on hypernym-concat-bananas-to-dogs-double (same layout as run_ifeval_concat.sh).
#
...
#
# Evaluates each variant on the explicit hypernym test task list from
# scripts/semi_supervised_eval_runs_hypernym.sh (v2 list; magnifying glasses excluded per that file).
#
# Usage:
#   ./run_hypernym_concat.sh <NUM_GPUS> [--model MODEL] [--task TASK] [--tc]
...
TASK_CONCAT="hypernym-concat-bananas-to-dogs-double"
USE_TC=0
SEMI_MODE="none"
SEMI_RATIO="0.1"
shift || true
...
if [ -z "$NUM_GPUS_INPUT" ]; then
    ...
    echo "  --task: training task name (default: hypernym-concat-bananas-to-dogs-double)"
