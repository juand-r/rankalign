#!/bin/bash
# Extra basetyp / basetypneg evals for the rosch-furniture-and-bird quick-iter
# models. The main eval script (run_eval_rosch_online_quickiter.sh) restricted
# basetyp to OFFLINE-trained models (matched-eval rule). This follow-up adds
# basetyp/basetypneg to:
...
# variants. (basetyp is the most honest cross-model reference because P_base
# is fixed; --self-typicality drifts per fine-tune.)
#
# Designed to be launched as a separate Slurm job in parallel with the
# matched-eval job (37250).
#
# All eval calls pass --validator-log-odds (consistent with the matched-eval
# script).
#
# Usage:
#   run 1 1 bash scripts/run_eval_rosch_online_quickiter_basetyp_extra.sh
...
# Per-variant TC tags MISSING from the main eval job (37250).
# Tags from {basetyp, basetypneg}.
declare -A EVAL_TC
EVAL_TC[1_baseline]="basetyp basetypneg"     # no-TC at train: both
EVAL_TC[3_onSelfTC]="basetyp"                # online self-TC: cross-comparable basetyp
EVAL_TC[4_onPairs]="basetyp basetypneg"      # no-TC at train: both
EVAL_TC[5_bothOnSelf]="basetyp"              # online self-TC: cross-comparable basetyp
EVAL_TC[6_SFT]="basetyp basetypneg"          # no-TC at train: both
EVAL_TC[8_onNegTC]="basetypneg"              # online neg-TC: cross-comparable basetypneg
EVAL_TC[9_bothOnNeg]="basetypneg"            # online neg-TC: cross-comparable basetypneg
