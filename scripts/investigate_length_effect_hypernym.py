...
# -*- coding: utf-8 -*-
"""
Investigate the length effect in typicality correction for HYPERNYM tasks.

Adapted from investigate_length_effect.py for hypernym data structure:
  - 8 hypernym tasks (bananas, bazookas, cabinets, cars, chairs, crows, diapers, dogs)
  - CSV pattern: scores_v6-google_gemma-2-2b_hypernym-NOUN_test_v2_log-odds_evaltc_*.csv

Note: Hypernym completions are mostly 1-2 tokens, so length effects will be
smaller than in plausibleqa. Still worth running for completeness.
"""
...
def load_all_hypernym(outputs_dir, self_typicality=False):
    """Load all hypernym CSVs into a single DataFrame with prompt_id column."""
    ...
    for noun, fp in sorted(seen.items()):
        ...
        if df['gpt4_ground_truth'].dtype == object:
            df['gpt4_ground_truth'] = df['gpt4_ground_truth'].str.strip().str.lower().map(
                {'yes': 1, 'no': 0}).fillna(0).astype(int)
        all_rows.append(df)

    df = pd.concat(all_rows, ignore_index=True)
    print(f"Loaded {len(df)} rows across {df['prompt_id'].nunique()} hypernym tasks")
    return df
...
def analyze_correlations(df, outputs_dir, suffix=''):
    """Pairwise correlations between gen, typ, len, val — pooled and within-task."""
    ...
    print(f"\n--- Within-task Pearson correlations (averaged across {n_tasks} tasks) ---")
    ...
    for ax, mat, title in [(axes[0], pooled_corr, f'Pooled (N={n_total})'),
                            (axes[1], within_corr, f'Within-task (mean of {n_tasks})')]:
    ...
    fig.suptitle('Correlation Structure: gen, typ, len, val (Hypernym)', fontsize=14, y=1.02)
    plt.tight_layout()
    plt.savefig(str(Path(outputs_dir) / f'length_correlation_matrix_hypernym{suffix}.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
    print(f"\nSaved: length_correlation_matrix_hypernym{suffix}.png")
...
def analyze_redundancy(df, outputs_dir, suffix=''):
    ...
    print(f"  r(typ, len) = {r_typ_len:.4f}, R² = {r_typ_len**2:.4f}")
    ...
    print(f"  Within-task r(typ, len): mean={np.mean(within_r_typ_len):.4f}")
    print(f"  Within-task R²: mean={r2_within:.4f}")

    if r2_within < 0.5:
        print(f"  CONCLUSION: Length is NOT merely a proxy for typ (R²={r2_within:.3f})")
    else:
        print(f"  CONCLUSION: Length is substantially redundant with typ (R²={r2_within:.3f})")
    ...
    ax.set_ylabel('Typicality (GPT-2 log P(y))')
    ax.set_title('Pooled: typ vs len (Hypernym)')
    ax.legend()
    ...
    ax.set_xlabel('Within-task r(typ, len)')
    ax.set_ylabel('Count (tasks)')
    ax.set_title('Distribution of within-task r(typ, len)')
    ax.legend()

    plt.tight_layout()
    plt.savefig(str(Path(outputs_dir) / f'length_redundancy_analysis_hypernym{suffix}.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
    print(f"Saved: length_redundancy_analysis_hypernym{suffix}.png")
...
def grid_search_alpha_c(df, outputs_dir, suffix=''):
    ...
    plt.tight_layout()
    plt.savefig(str(Path(outputs_dir) / f'length_grid_search_heatmap_hypernym{suffix}.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
    print(f"\nSaved: length_grid_search_heatmap_hypernym{suffix}.png")
...
# =========================================================================
# Analysis 4: Per-task length coefficient
# =========================================================================

def analyze_per_prompt_length(df, outputs_dir, suffix=''):
    """Fit val ~ gen + typ + len per task, examine length coefficient."""

    print(f"\n{'=' * 80}")
    print("ANALYSIS 4: PER-TASK LENGTH COEFFICIENT")
    print(f"{'=' * 80}")
    ...
    print(f"\nPer-task length coefficient (b_len) distribution:")
    ...
    print(f"\n{'Task':>10} {'n':>4} {'b_gen':>8} {'b_typ':>8} {'b_len':>8} "
          f"{'r_with':>8} {'r_without':>8} {'delta_r':>8}")
    print("-" * 75)
    for _, row in df_results.sort_values('b_len').iterrows():
        print(f"{row['prompt_id']:>10} {row['n']:>4} {row['b_gen']:>8.3f} "
    ...
    ax.set_ylabel('Count (tasks)')
    ax.set_title('Per-task b_len (Hypernym)')
    ax.legend()
    ...
    ax.set_title('Does length help per-task?')
    ...
    ax.set_yticklabels(sorted_df['prompt_id'], fontsize=9)
    ax.axvline(0, color='black', linestyle='-', linewidth=1)
    ax.set_xlabel('Length coefficient (b_len)')
    ax.set_title('b_len by task (sorted)')

    plt.tight_layout()
    plt.savefig(str(Path(outputs_dir) / f'length_per_prompt_analysis_hypernym{suffix}.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
    print(f"\nSaved: length_per_prompt_analysis_hypernym{suffix}.png")
...
def main():
    parser = argparse.ArgumentParser(description='Investigate length effect for hypernym')
    ...
    df = load_all_hypernym(args.outputs_dir, self_typicality=args.self_typicality)
