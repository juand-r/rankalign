...
# -*- coding: utf-8 -*-
"""
Extended mixed-effects analysis of typicality correction for HYPERNYM tasks.

Adapted from mixed_effects_extended.py for the hypernym task structure:
  - 8 hypernym tasks (bananas, bazookas, cabinets, cars, chairs, crows, diapers, dogs)
  - Each task acts as a "prompt" in the mixed-effects model grouping
...
def load_all_hypernym(outputs_dir, self_typicality=False):
    ...
    # Deduplicate: keep latest file per noun
    seen = {}
    for fp in files:
...
# =========================================================================
# Model specification framework (same as plausibleqa version)
# =========================================================================
...
def fit_all_models_full(df, models):
    ...
    print(f"\n{'=' * 80}")
    print(f"FULL-DATA MODEL FITS (all {n_tasks} hypernym tasks)")
    print(f"{'=' * 80}")
...
def loo_evaluation(df, models):
    """LOO evaluation: for each hypernym task, refit on remaining tasks, predict on held-out."""
    prompts = sorted(df['prompt_id'].unique())
    n_prompts = len(prompts)

    print(f"\n{'=' * 80}")
    print(f"TRUE LOO EVALUATION ({n_prompts} hypernym tasks)")
    print(f"{'=' * 80}")
    ...
    for i, eval_pid in enumerate(prompts):
        ...
        print(f"  Task {i+1}/{n_prompts} ({eval_pid}) done "
              f"({elapsed:.0f}s elapsed, ~{eta:.0f}s remaining)")

    for mname, nfail in model_failures.items():
        if nfail > 0:
            print(f"  WARNING: {mname} failed on {nfail}/{n_prompts} tasks")
...
def main():
    parser = argparse.ArgumentParser(description='Mixed-effects typicality analysis for hypernym')
    ...
    if args.save_csv:
        ...
        csv_path = str(Path(args.outputs_dir) / f'mixed_effects_hypernym{suffix}.csv')
