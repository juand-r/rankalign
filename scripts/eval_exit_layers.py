...
from utils import get_L_prompt, get_final_logit_prob, get_completion_token_logprobs, get_completion_token_logprobs_exit, _get_model_lm_components, get_model_input_device
...
def get_completion_logprobs_all_exits(
    ...
    Uses logit lens (final norm + lm_head applied to intermediate hidden states).
    All exit fractions are captured from one call with output_hidden_states=True.

    Args:
        exit_fractions: list of float fractions, e.g. [0.25, 0.5, 0.75]

    Returns:
        full_logprobs: Tensor [n_completion_tokens] — full model log probs
        exit_logprobs: dict {frac: Tensor [n_completion_tokens]} for each frac
    """
    ...
    with torch.no_grad():
        ...
        if outputs.hidden_states is None:
            raise RuntimeError(
                ...
                "The model may not propagate this flag."
            )
        ...
        for frac, exit_idx in exit_indices.items():
            ...
            exit_log_probs_map[frac] = exit_logits.log_softmax(-1).squeeze(0)  # [seq_len, vocab]

        full_tok, exit_tok = [], {frac: [] for frac in exit_fractions}
        for k, tok_id in enumerate(completion_ids):
            t = prefix_len + k - 1
            if t < 0 or t >= log_probs.shape[0]:
                continue
            full_tok.append(log_probs[t, tok_id].item())
            for frac in exit_fractions:
                exit_tok[frac].append(exit_log_probs_map[frac][t, tok_id].item())

        full_tensor = torch.tensor(full_tok)
        exit_tensors = {frac: torch.tensor(exit_tok[frac]) for frac in exit_fractions}
        return full_tensor, exit_tensors
...
def make_negated_gen_prompt(item, task, make_prompt, gen_shots='zero'):
    ...
    if task_config is not None and "make_negated_prompt" in task_config:
        negated = task_config["make_negated_prompt"](
            item=item, task=task, make_prompt=make_prompt, gen_shots=gen_shots
        )
    ...
    if is_hypernym_task(task):
        ...
        # NOTE: codecontests/humaneval negated prompts should be handled via the
        # task registry callback (make_negated_prompt). Reaching this branch means
        # the task has no callback.
...
def main(args):
    ...
    P_disc = []
    gen_sum_logprobs = []
    exit_sum_logprobs = {frac: [] for frac in (args.exit_layers or [])}  # {frac: [sum_logp, ...]}
    disc_probs = []
    ...
    for item in tqdm(LL):
        ...
        if use_full_completion_logprobs:
            ...
            if args.exit_layers:
                gen_token_logprobs, exit_logprobs_dict = get_completion_logprobs_all_exits(
    ...
    # Compute exit-layer (logit lens) corrected scores if --exit-layers was set
    # gen_scores_exit: {frac: list[float]} — ACD-corrected scores per exit fraction
    gen_scores_exit = {}
    if exit_sum_logprobs:
        beta = args.exit_layer_beta
        print(f"\nExit-layer corrections (beta={beta}):")
        print(f"  Raw gen mean: {sum(gen_scores_raw)/len(gen_scores_raw):.4f}")
        for frac in sorted(exit_sum_logprobs.keys()):
            exit_lps = exit_sum_logprobs[frac]
            scores = [g - beta * e for g, e in zip(gen_scores_raw, exit_lps)]
            gen_scores_exit[frac] = scores
            print(f"  fraction={frac}: exit_lp_mean={sum(exit_lps)/len(exit_lps):.4f}  corrected_mean={sum(scores)/len(scores):.4f}")
    ...
    if args.train:
        ...
        if args.save_scores_csv and is_hypernym_task(task):
            ...
            if task_config is None or "csv_header" not in task_config or "csv_row_builder" not in task_config:
                raise ValueError(
                    f"Task '{task}' missing csv_header/csv_row_builder in task registry."
                )

            scores_csv_filename = build_csv_filename(
                outputs_dir, self_prefix, modelname, task, "train", args, eos_suffix
            )
            strategy = build_strategy_string(
                args, gen_shots, disc_shots, use_full_completion_logprobs
            )
            if args.exit_layers:
                strategy += "_exits_" + "_".join(f"{f}" for f in sorted(args.exit_layers))

            # Extend header with exit-layer columns (one pair per fraction, sorted)
            csv_header = list(task_config["csv_header"])
            sorted_fracs = sorted(gen_scores_exit.keys())
            for frac in sorted_fracs:
                tag = f"{int(round(frac * 100)):03d}"  # e.g. 025, 050, 075
                csv_header += [f"gen_score_exit{tag}", f"gen_score_exit{tag}_lenorm"]
            ...
            for i, item in enumerate(LL):
                ...
                for frac in sorted_fracs:
                    exit_score = gen_scores_exit[frac][i]
    ...
    #NOTE: This is what FUTURE TASKS SHOULD USE
        ...
        split = "train" if args.train else "test"
        scores_csv_filename = build_csv_filename(
            outputs_dir, self_prefix, modelname, task, split, args, eos_suffix
        )
        strategy = build_strategy_string(
            args, gen_shots, disc_shots, use_full_completion_logprobs
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exit-layers", type=float, nargs='+', default=None, help="logit-lens exit layers as fractions of total layers (e.g. --exit-layers 0.25 0.5 0.75). All captured in a single forward pass.")
    parser.add_argument("--exit-layer-beta", type=float, default=1.0, help="beta for ACD-style correction: gen_score - beta * exit_score. Applied to all exit layers. Default: 1.0")
